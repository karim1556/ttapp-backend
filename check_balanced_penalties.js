...
async function main() {
  const result = await generateAllSchedules({
    ...
    fillCompact: true,
    optimizerRuns: 10 // Let's run a few attempts to inspect
  });

  console.log('\n=== GENERATION DONE ===');
  console.log('Attempts:', result.optimization.attempts);
  console.log('Placed Lectures:', result.optimization.placedLectures);
  console.log('Unplaced Lectures:', result.optimization.unplacedLectures);
  console.log('Best Score:', result.optimization.bestScore);
  
  // We can query the database to analyze the generated timetables
  ...
  // Analyze class-by-class, day-by-day load
